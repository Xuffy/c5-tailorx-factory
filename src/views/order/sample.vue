  ...
  <div class="order-list-info" v-bind:class="{'print-mode':showPrintPreview}">
    <div class="oli-box">
      <blockquote class="quote-box">
        ...
        <div style="position: absolute;right: 20px;top: 8px">
          ...
          <el-button type="primary"
                     ...
                     @click="showOperateFormBox(2,orderInfo.rework == 1 ? 1 : 0)">
            分配
          </el-button>
          <el-button type="primary"
          ...
          <el-button type="primary" style="background-color: #ffffff;color: #666;border-color: #e2e2e2"
                     @click="$router.push({path:'/order/samples'})">返回
          </el-button>
        </div>
      </blockquote>
      ...
      <fieldset class="field-box basic-info" id="basic-info">
        <legend>基本信息</legend>
        <div style="font-size: 16px;color: #FF6633">
          定制数量：
          <span v-for="item in orderInfo.sampleSizeCount" v-if="item.count"
                v-text="sampleSize[item.size] +' × '+item.count+'件'"
                style="display: inline-block;margin-right: 20px;"></span>
        </div>
        <ul>
          <li>订单编号：<span v-text="orderInfo.sampleNo"></span></li>
          <li>订单类型：<span v-text="orderInfo.fitting"></span></li>
          <li>服装类型：
            <span
              v-text="((orderInfo.genderName ? '-' + orderInfo.genderName : '') +
          (orderInfo.styleName ? '-' + orderInfo.styleName : '') +
          (orderInfo.seasonName ? '-' + orderInfo.seasonName : '')).replace('-','')"></span>
          </li>
          <li>订单等级：D</li>
          <li>工艺单创建时间：<span v-text="orderInfo.createTimeStr"></span></li>
        </ul>
        <ul>
      ...
      <fieldset class="field-box" id="field-img" style="margin: 10px;" v-bind:class="{'tx-hide':showPrintPreview}">
        <legend>版式图下载</legend>
        <p style="cursor: pointer;color: rgb(0, 102, 204)"
           v-for="item in orderInfo.cadUrlSize">
      ...
      <fieldset class="field-box" v-bind:class="{'tx-hide':showPrintPreview}" style="margin: 10px;">
        <legend>处理信息</legend>
        <el-collapse v-model="collapseShow">
          <el-collapse-item v-for="(item,index) in orderInfo.sampleLogs" :key="index" v-if="item.tacheName"
                            :name="index">
            <template slot="title">
              <span style="font-weight: bold" v-text="item.tacheName"></span>
      ...
      <fieldset class="field-box designInfo" id="design-info" style="margin: 10px;">
        ...
        <div class="img-box">
          <img v-bind:style="{width: showPrintPreview ? '95%' : '19%'}" v-for="item in orderInfo.designUrl"
               v-bind:src="item" v-img-preview="item">
        </div>

        <p class="title">设计师要求</p>
        <div class="child-box">
          <p v-html="orderInfo.designDescription" style="width: 70%"></p>
        </div>
        ...
        <div class="realData">
          <table class="tx-table" width="40%" style="float: left;">
            <tr>
              <td width="50%">上衣</td>
              <td width="50%">成衣尺寸</td>
            </tr>
          ...
          <table class="tx-table" width="40%" style="float: left;border-left: none">
            <tr>
        ...
        <div class="realData">
          <table class="tx-table" width="40%" style="float: left;">
            <tr v-for="item in orderInfo.first_storeCustomerBodyList">
              ...
              <td width="50%" v-text="item.size || '-'"></td>
            </tr>
          </table>
          <table class="tx-table" width="40%" style="float: left;border-left: none">
        ...
        <table class="tx-table" width="80%">
          ...
          <tr v-for="item in orderInfo.material">
            <td v-text="item.serialNumber"></td>
            <td v-text="item.barCode || item.colourNumber"></td>
            <td>
        ...
        <table class="tx-table" width="80%" style="margin-bottom: 20px">
          <tr>
            <td width="20%">编号</td>
            <td width="20%">色号</td>
            <td width="40%">图片</td>
          ...
          <tr v-for="item in orderInfo.lining">
            <td v-text="item.serialNumber"></td>
            <td v-text="item.barCode || item.colourNumber"></td>
            <td>
              <img style="max-height: 80px"
        ...
        <table class="tx-table" width="80%" style="margin-bottom: 20px">
          <tr>
            <td width="20%">编号</td>
            <td width="20%">色号/名称</td>
            <td width="40%">图片</td>
            <td width="20%">备注</td>
          </tr>
          <tr v-for="item in orderInfo.accessory">
            <td v-text="item.serialNumber || item.match"></td>
            <td v-text="item.barCode || item.rawMaterials"></td>
            <td>
              <img style="max-height: 80px"
                   v-if="item.pictrue || item.picture"
                   v-bind:src="item.pictrue || item.picture"
    ...
    <ul class="tag-box" v-bind:class="{'tx-hide':showPrintPreview}">
...
<script>
  ...
  export default {
    ...
    data: function () {
      return {
        sampleSize: ['均码', 'XXS码', 'XS码', 'S码', 'M码', 'L码', 'XL码', 'XXL码', 'XXXL码'],
        collapseShow: [],
        taskOrderStep: [],
    ...
    methods: {
      getInfo: function () {
        let _this = this;
        // 订单详情
        this.ajax.post('/sampleOrder/detail', {sampleId: this.$route.query.id})
          .then(function (data) {
            // 订单状态
            ...
            data.sampleLogs = _.filter(data.sampleLogs, function (val) {
              if (val.img) {
            ...
            _this.orderInfo = _this.filterTaskData(data);
            _this.collapseShow = _.range(data.sampleLogs.length);
      ...
      filterTaskData: function (data) {
        ...
        // 电子量体数据过滤
        _.map(JSON.parse(data.designBody), function (val) {
        ...
        // 用户真实量体数据
        data.customerBody = JSON.parse(data.customerBody);
        _.map(data.customerBody, function (val, index) {
          if (index < data.customerBody.length / 2) {
        ...
        data.cadUrlSize = JSON.parse(data.cadUrlSize || '{}');
      ...
      showOperateFormBox: function (tIndex, type) {
        ...
        this.operateForm.sampleId = this.orderInfo.id;
      ...
      submitOperateForm: function () {
        ...
        if (this.operateForm.showType === 0 || this.operateForm.showType === 1) {
          ...
          this.ajax.post('/sampleOrder/receiveOrderInfo', this.operateForm)
            .then(function () {
              Message({
                message: '已{0}'.format(_this.operateForm.type === 1 ? '拒绝，等待门店修改' : '接收成功'),
                type: 'success'
              });
              _this.$router.push({path: '/order/samples'});
            });
        }
        // 分配订单
        ...
        else if (this.operateForm.showType === 7) {
      ...
      showUpdateOperateBox: function () { // 获取显示生产状态
        ...
        this.orderStepFrom.sampleId = this.orderInfo.id;

        this.ajax.get('/sampleOrder/findStatus', {sampleId: this.orderInfo.id})
          .then(function (data) {
            data = data || [];
        ...
        this.ajax.post('/sampleOrder/updateStatus', this.orderStepFrom)
          .then(function () {
            ...
            if (tacheId === 7 || tacheId === 5) {
              return _this.$router.push({path: '/order/samples'});
            }
            _this.getInfo();
